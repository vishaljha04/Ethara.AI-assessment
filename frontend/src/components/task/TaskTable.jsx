...
export function TaskTable({ tasks, onStatusChange, loading }) {
  if (loading) {
    return <div className="rounded-3xl border border-slate-200 bg-white p-8">Loading tasks…</div>
  }

  if (!tasks.length) {
    return <div className="rounded-3xl border border-slate-200 bg-white p-8 text-slate-600">No tasks found yet. Create a task to get started.</div>
  }
